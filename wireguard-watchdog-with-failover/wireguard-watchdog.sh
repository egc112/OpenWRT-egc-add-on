...
# installation:
# 1. Copy wireguard-watchdog.sh from https://raw.githubusercontent.com/egc112/OpenWRT-egc-add-on/main/wireguard-watchdog-with-failover/wireguardg-watchdog.sh to /usr/share
#    either with, from commandline (SSH): curl -o /usr/share/wireguard-watchdog.sh https://raw.githubusercontent.com/egc112/OpenWRT-egc-add-on/main/wireguard-watchdog-with-failover/wireguardg-watchdog.sh
...
set_active(){
	...
	if [[ ${RESTARTNETWORK+x} ]]; then
		echo "WireGuard watchdog: restarting network with active interface $wg_activetunnel"
