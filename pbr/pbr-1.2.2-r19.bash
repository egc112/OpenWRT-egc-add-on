...
readonly PKG_VERSION='1.2.2-r19'
...
pbr_get_gateway4() {
	...
	if [ -z "$gw" ] || [ "$gw" = '0.0.0.0' ]; then
		...
		#{ [ -z "$gw" ] && ! ip address show dev "$dev" 2>/dev/null | grep -q "POINTOPOINT"; } && json add error 'warningInterfaceRoutingUnknownGateway' "IPv4 interface:[$iface]; device:[$dev]"
...
ubus_get_data() { __ubus_get "@.${packageName}.instances.main.data.${1}[*]"; }  # use [*] otherwise ti will return an empty arry
