...
readonly PKG_VERSION='1.1.8-r31-egc-6'
readonly packageCompat='13'
...
is_config_enabled() {
	#logger -t "$packageName" "EGC-11 is_config_enabled : cfg=$1"
	...
	#logger -t "$packageName" "EGC-12 is_config_enabled  _cfg_enabled=$_cfg_enabled"
	return "$_cfg_enabled"
}
uci_get_device() {
	local __tmp
...
check_dnsmasq_nftset() {
	logger -t "$packageName" "EGC-23: processing check_dnsmasq_nftset"
	local o;
	check_nft || return 1
...
load_environment() {
	_system_health_check() {
		...
		logger -t "$packageName" "EGC-101 before _system_health_check return=$_ret "
		return "$_ret"
	}
	local param="$1" validation_result="$2"
	logger -t "$packageName" "EGC-51 load environment param=$1 validation_result=$2"
	case "$param" in
		on_boot|on_start)
...
nft_file() {
	logger -t "$packageName" "EGC-07 nft_file 1=$1"
	local i
	[ -x "$nft" ] || return 1
	case "$1" in
		add|add_command)
...
state() {
	...
	shift 2
	local value="$*"
	local line error_id error_extra array_name label
	case "$action" in
		add)
			line="$(eval echo "\"\$$param\"")"
			value="${line:+$line\n}${value}"
# shellcheck disable=SC3037
			eval "$param=\"$(echo -e "$value")\""
		;;
		json)
			json_init
...
resolver() {
		logger -t "$packageName" "EGC-20 resolver cfg=$1" 
...
policy_routing() {
	...
	logger -t "$packageName" "EGC-14 policy routing name=$1 iface=$2 src_addr=$3 src_port=$4 dest_addr=$5 dest_port=$6 proto chain uid=$9 "
	if [ -z "$ipv6_enabled" ] && \
...
dns_policy_process() {
	local i j uid="$1"
	logger -t "$packageName" "EGC-10 policy_process  uid=$uid"
...
policy_process() {
	#logger -t "$packageName" "EGC-13 policy process" 
	local i j uid="$1"
	...
	for i in $src_addr; do
		if ! str_contains "$processed_value_src_addr" "$i"; then
			processPolicyError='true'
			state add 'errorSummary' 'errorPolicyProcessUnknownEntry' "$name: $i"
		fi
	done
...
process_interface() {
	...
	#logger -t "$packageName" "EGC-08 process interface $iface $action $reloadIface"
...
#	ubus -t 30 wait_for network.interface 2>/dev/null
	pbr_boot_flag=1
	logger -t "$packageName" "EGC-01: boot"
	rc_procd start_service 'on_boot' && service_started 'on_boot'
	#rc_procd start_service 'on_boot' && { service_started 'on_boot'; pbr_boot_flag=1; }
	#rc_procd start_service 'on_boot' && { pbr_boot_flag=1; service_started 'on_boot'; }
}
...
start_service() {
	...
	logger -t "$packageName" "EGC-02: start_service pbr_boot_flag=$pbr_boot_flag; param=$param"
	[ -n "$pbr_boot_flag" ] && return 0
	#r31
	[ "$param" = 'on_boot' ] && return 0
	#r31
	load_environment "${param:-on_start}" "$(load_validate_config)" || return 1
	is_wan_up "$param" || { output "${_ERROR_}: Uplink/WAN interface is still down, increase value of 'procd_boot_trigger_delay' option!\n";  return 1; }
	logger -t "$packageName" "EGC-04: WAN is up proceeding"
	...
	logger -t "$packageName" "EGC-06 serviceStartTrigger=$serviceStartTrigger"
	...
	case $serviceStartTrigger in
		...
		on_reload|on_start|*)
			...
			if is_config_enabled 'policy'; then
				logger -t "$packageName" "EGC-09 is_config_enabled policy"
...
service_started() {
	#r31
	logger -t "$packageName" "EGC-03: service_started pbr_boot_flag=$pbr_boot_flag; param=$param"
	[ -n "$pbr_boot_flag" ] && return 0
	#[ "$param" = 'on_boot' ] && return 0
	#r31
	if nft_file 'exists'; then
...
service_triggers() {
	logger -t "$packageName" "EGC-03.1 Starting service_triggers"
	local n
...
load_validate_config() {
	uci_load_validate "$packageName" "$packageName" "$1" "${2}${3:+ $3}" \
		...
		'procd_boot_trigger_delay:range(1000,90000):6000' \
