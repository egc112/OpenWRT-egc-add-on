...
readonly PKG_VERSION='1.2.2-r15.9'
...
get_text() {
	...
	case "$r" in
		...
		warningInterfaceRoutingUnknownGateway) printf "Gateway for device '%s' is not up yet" "$1";;
...
nftset() {
	...
	case "$command" in
		...
		add_dnsmasq_element)
			...
			# Skip if this exact IPv4 set is already referenced for this domain
			grep -q "^nftset=/${escaped_param}/.*${nftset4}" "$packageDnsmasqFile" && return 0
			if grep -q "^nftset=/${escaped_param}/" "$packageDnsmasqFile"; then
				# Domain already has an entry — append only missing sets
				# Strip comment, append new_spec, preserving one clean line
				local append_spec="$new_spec"
				# If nftset6 already present, only append the v4 part
				if [ -n "$nftset6" ] && grep -q "^nftset=/${escaped_param}/.*${nftset6}" "$packageDnsmasqFile"; then
					append_spec="4#inet#${nftTable}#${nftset4}"
				fi
				sed -i "/^nftset=\/${escaped_param}\// s| # [^|]*$||; \
						/^nftset=\/${escaped_param}\// s|$|,${append_spec}|" \
