...
readonly PKG_VERSION='1.1.8-r25-egc-3'
readonly packageCompat='12'
...
policy_process() {
		...
		if [ -z "$src_addr" ] || { [ -n "$src_addr" ] && [ -n "$filtered_value_src_addr" ]; }; then
			...
			for filter_group_dest_addr in $filter_list_dest_addr; do
				...
				if [ -z "$dest_addr" ] || { [ -n "$dest_addr" ] && [ -n "$filtered_value_dest_addr" ]; }; then
					...
					#logger "pbr EGC-101: processed_value_src_addr=[$processed_value_src_addr] " 
					#processed_value_src_addr="${processed_value_src_addr:+$processed_value_src_addr }$filtered_value_src_addr"
					#l#ogger "pbr EGC-102: processed_value_src_addr=[$processed_value_src_addr] " 
				fi
			done
		processed_value_src_addr="${processed_value_src_addr:+$processed_value_src_addr }$filtered_value_src_addr"
		#logger "pbr EGC-102: processed_value_src_addr=[$processed_value_src_addr] " 
		fi
	done

	for i in $src_addr; do
		#logger "pbr EGC-103: policy routing: i=[$i]; src_addr=[$src_addr]; processed_value_src_addr=[$processed_value_src_addr] " 
	...
	for i in $dest_addr; do
		#logger "pbr EGC-104: policy routing: i=[$i]; dest_addr=[$dest_addr]; processed_value_dest_addr=[$processed_value_dest_addr] " 
