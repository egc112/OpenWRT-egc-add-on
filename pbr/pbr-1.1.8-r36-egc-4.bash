...
readonly PKG_VERSION='1.1.8-r36-egc-4'
...
#							fi
							while read -r i; do
								...
								# remove only sec but keep expire
								#i="$(echo "$i" | sed -E 's/ proto kernel//; s/( expires -?[0-9]+)sec/\1/')"
								#i="$(echo "$i" | sed 's/ proto kernel//')"
								#echo "$i" | grep -q "proto kernel" && continue
								#echo "$i" | grep -q -E '^[23]00.:.*proto kernel' && continue
								#echo "$i" | grep -q -E '^[23][0-9a-fA-F]{0,3}:.*proto kernel' && continue
								# shellcheck disable=SC2086
