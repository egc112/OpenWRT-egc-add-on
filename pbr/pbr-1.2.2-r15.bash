...
readonly PKG_VERSION='1.2.2-r14'
...
pbr_get_gateway4() {
	...
	if [ -z "$gw" ] || [ "$gw" = '0.0.0.0' ]; then
		gw="$(ip -4 route show dev "$dev" table all 2>/dev/null | awk '{for (i=1; i<=NF; i++) if ($i=="via") {print $(i+1); exit}}')"    # use table all in case of netifd where default routes are not present in main table
		...
		{ [ -z "$gw" ] && ! ip address show dev "$dev" 2>/dev/null | grep -q "POINTOPOINT"; } && json add error 'errorInterfaceRoutingUnknownGateway' "$dev"
	fi
	eval "$1"='$gw'
}
pbr_get_gateway6() {
	...
	if [ -z "$gw" ] || [ "$gw" = '::/0' ] || [ "$gw" = '::0/0' ] || [ "$gw" = '::' ]; then
		gw="$(ip -6 route show dev "$dev" table all 2>/dev/null | awk '{for (i=1; i<=NF; i++) if ($i=="via") {print $(i+1); exit}}')"    # use table all in case of netifd where default routes are not present in main table
		...
		# Raise error if no gw and not point-to-point link
		{ [ -z "$gw" ] && ! ip address show dev "$dev" 2>/dev/null | grep -q "POINTOPOINT"; } && json add error 'errorInterfaceRoutingUnknownGateway' "$dev"
	fi
	eval "$1"='$gw'
}
...
get_text() {
	...
	case "$r" in
		...
		errorInterfaceRoutingUnknownGateway) printf "Unknown Gateway for device '%s'" "$1";;
	...
	case "$action" in
		create)
			...
			if [ -n "$dev4" ]; then
				...
				elif ip address show dev "$dev4" 2>/dev/null | grep -q "POINTOPOINT"; then     # might need to set this rule as first to prevent rogue gatewy deteciton for point-to-point links
			...
			if [ -n "$ipv6_enabled" ] && [ -n "$dev6" ]; then
				...
				elif ip address show dev "$dev6" 2>/dev/null | grep -q "POINTOPOINT"; then     # might need to set this rule as first to prevent rogue gatewy deteciton for point-to-point links
