...
readonly PKG_VERSION='1.1.8-r18-egc-policyerror-4'
...
policy_process() {
	...
	if [ -n "$processPolicyError" ] || [ -n "$processSrcFail" ] || [ -n "$processDestFail" ]; then
		...
		if [ -n "$processSrcFail" ]; then
			logger -p user.err -t "$packageName" "Policy $name failing with error in source $processSrcFail"
		fi
		if [ -n "$processDestFail" ]; then
			logger -p user.err -t "$packageName" "Policy $name failing with error in destination $processDestFail"
		fi
	else
		output_ok
	fi
