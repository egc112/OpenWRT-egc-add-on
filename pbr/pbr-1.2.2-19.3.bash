...
readonly PKG_VERSION='1.2.2-r19.3'
...
process_interface() {
	...
	# Open VPN proto might work with network_get_device but not with config settings
	
	#network_get_device dev4 "$iface" || network_get_physdev dev4 "$iface" || uci_get_device dev4 "$iface" || uci_get_dev dev4 "$iface" 
	#network_get_device dev4 "$iface" || network_get_physdev dev4 "$iface" || uci_get_device dev4 "$iface" || { is_ovpn "$iface" && uci_get_dev dev4 "$iface";  } || true
...
start_service() {
	...
	# Build ifacesTriggers BEFORE is_wan_up — enumerate_interface only
...
service_triggers() {
	...
	# emit — so emitting fewer triggers in any branch silently wipes
