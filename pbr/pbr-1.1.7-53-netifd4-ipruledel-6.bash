...
readonly PKG_VERSION='pbr-1.1.7-53-netifd4-ipruledel-6'
...
# shellcheck disable=SC2317
setup_netifd() {
	local waniface waniface6 param="$1"
	#local param="$1"
	_pbr_iface_setup() {
		local iface="${1}" waniface="$2" waniface6="$3"
		if is_supported_interface "${iface}"; then
			...
			if [ "$iface" = "${waniface:-wan}" ]; then
				uci_set 'network' "${waniface6:-wan6}" 'ip6table' "${ipTablePrefix}_${iface%6}"
			else
			...
			_pbr_netifd_set_defrt() {
				local IPV="$1" waniface="$2"
				# skip wan Interface
				[ "$iface" = "${waniface:-wan}" ] && return 0
				output "Setting up ${packageName} default IPv${IPV} route for table ${ipTablePrefix}_${iface%6} }"
				uci_add 'network' "route${IPV%4}" "${ipTablePrefix}_${iface%6}_${IPV}"
				uci_set 'network' "${ipTablePrefix}_${iface%6}_${IPV}" 'interface' "${iface}"
				if [ "$IPV" = "6" ]; then
			...
			_pbr_netifd_set_defrt 4 "$waniface"
			[ $(uci -q get pbr.config.ipv6_enabled) -eq 1 ] && _pbr_netifd_set_defrt 6 "$waniface"
		fi
	}
	_pbr_default_route_setup() {
		local waniface waniface6
		# network_find_wan finds the actual default interface so could be different from wan/wan6
		[ -z "$waniface" ] && { network_flush_cache; network_find_wan waniface; }
		[ -z "$waniface6" ] && { network_flush_cache; network_find_wan6 waniface6; }
		output "Setting up ${packageName} default IPv4 route via ${waniface} ${param:+($param) }"
		...
		uci_set network pbr_default lookup "${ipTablePrefix}_${waniface}"
		uci_set network pbr_default priority "40000"
		output_okbn
		output "Setting up ${packageName} default IPv6 route via ${waniface6%6} ${param:+($param) }"
		...
		# if ipv6 default route is via wan6 then the routes can be found in pbr_wan there is no pbr_wan6
		uci_set network pbr_default6 lookup "${ipTablePrefix}_${waniface6%6}"
	...
	sync
	config_load 'network'
	waniface="$(uci_get 'pbr' 'config' 'procd_wan_interface')"
	waniface6="$(uci_get 'pbr' 'config' 'procd_wan6_interface')"
	config_foreach _pbr_iface_setup 'interface' "$waniface" "$waniface6"
	_pbr_default_route_setup "$waniface" "$waniface6"
	uci_commit 'network'
	sync
