...
readonly PKG_VERSION='1.2.1-r69'
readonly packageCompat='23'
...
nft_file() {
	...
	case "$1" in
		...
		;;
		sed_temp|sed_temp_file)
			shift
			sed -i "$*" "$nftTempFile" >/dev/null 2>&1
		;;
		sed_netifd|sed_netifd_file)
			shift
			sed -i "$*" "$nftNetifdPermFile" >/dev/null 2>&1
		;;
	esac
}
nftset() {
...
netifd() {
	...
	_netifd_process_interface() {
		...
		if is_split_uplink; then
			if is_wan "$iface" || is_wan6 "$iface"; then
				if [ -n "$_uplinkMark" ] && [ -n "$_uplinkPriority" ] && [ -n "$_uplinkTID" ]; then
					_mark="$_uplinkMark"
			...
			case "$action" in
				install)
					...
					[ "$rt_name" = 'main' ] || echo "${_tid} ${rt_name}" >> "$rtTablesFile"
						nft_file 'sed_temp' "\#${_mark}#d"
					fi
					if ! nft_file 'match_temp' "${nftPrefix}_mark_${_mark}"; then
	...
	local _uplinkMark _uplinkPriority _uplinkTID
	...
	case "$action" in
		create)
			...
			if [ -n "$dev4" ]; then
				...
				if ! nft_file 'match_temp' "${nftPrefix}_mark_${mark}"; then
...
# shellcheck disable=SC2154
	status="$serviceName on $PRETTY_NAME."
	[ -n "$uplink_interface4" ] && status="$status Uplink (IPv4): ${uplink_interface4}/${dev4}/${uplinkGW4:-0.0.0.0}."
	[ -n "$uplink_interface6" ] && status="$status Uplink (IPv6): ${uplink_interface6}/${dev6}/${uplinkGW6:-::/0}."
