...
readonly PKG_VERSION='1.1.8-r20-egc-2'
readonly packageCompat='11'
readonly serviceName="$packageName $PKG_VERSION"
readonly packageConfigFile="/etc/config/${packageName}"
#readonly packageDebugFile="/etc/config/${packageName}.debug"
...
#is_host() { echo "$1" | grep -qE '^[a-zA-Z0-9][a-zA-Z0-9\-_]{0,61}[a-zA-Z0-9]$|^[a-zA-Z0-9]$'; }
#is_hostname() { echo "$1" | grep -qE '^([a-zA-Z0-9]([a-zA-Z0-9\-_]{0,61}[a-zA-Z0-9])?\.)*[a-zA-Z0-9]([a-zA-Z0-9\-_]{0,61}[a-zA-Z])?$'; }
#is_domain() { is_host "$1" || is_hostname "$1"; }
...
check_dnsmasq_nftset() {
	...
	echo "$(date)" >> "$packageDebugFile"
	echo "${o%This software*}" >> "$packageDebugFile"
...
policy_process() {
	...
	# TODO: check if src_addr and dest_addr do not containt unprocessable values

	local processSrcFail processDestFail filtered_value_src_addr_tot filtered_value_dest_addr_tot
	local filter_list_src_addr='phys_dev phys_dev_negative mac_address mac_address_negative domain domain_negative ipv4 ipv4_negative ipv6 ipv6_negative'
	...
	filtered_value_src_addr_tot=""
	for filter_group_src_addr in $filter_list_src_addr; do
		processSrcFail=""
		filtered_value_src_addr="$(filter_options "$filter_group_src_addr" "$src_addr")"
		[ -n "$filtered_value_src_addr" ] && filtered_value_src_addr_tot="$filtered_value_src_addr_tot $filtered_value_src_addr"
		#logger "pbr EGC-100: src_addr=$src_addr; filtered_value_src_addr=$filtered_value_src_addr; filtered_value_src_addr_tot=$filtered_value_src_addr_tot; filter_group_src_addr=$filter_group_src_addr"
		if [ -z "$src_addr" ] || { [ -n "$src_addr" ] && [ -n "$filtered_value_src_addr" ]; }; then
			[ -z "$dest_addr" ] && filter_list_dest_addr='none'
			filtered_value_dest_addr_tot=""
			processDestFail=""
			for filter_group_dest_addr in $filter_list_dest_addr; do
				...
				[ -n "$filtered_value_dest_addr" ] && filtered_value_dest_addr_tot="$filtered_value_dest_addr_tot $filtered_value_dest_addr"
				#logger "pbr EGC-102: dest_addr=$dest_addr; filtered_value_dest_addr=$filtered_value_dest_addr; filtered_value_dest_addr_tot=$filtered_value_dest_addr_tot; filter_group_dest_addr=$filter_group_dest_addr"
				if [ -z "$dest_addr" ] || { [ -n "$dest_addr" ] && [ -n "$filtered_value_dest_addr" ]; }; then
					...
					#logger "pbr EGC-103: policy routing: $name $interface $filtered_value_src_addr $src_port $filtered_value_dest_addr $dest_port $proto $chain $uid"
			...
			for addr in $dest_addr; do
				#logger "pbr EGC-110: FOUND DEST: addr=$addr; dest_addr=$dest_addr; filtered_value_dest_addr_tot=$filtered_value_dest_addr_tot"
				if ! str_contains_word "$filtered_value_dest_addr_tot" "$addr"; then 
					processDestFail="$processDestFail $addr"
					#logger "pbr EGC-120: FOUND DEST:  ERROR in $processDestFail"
				fi
			done
		fi
	done
	for addr in $src_addr; do
		#logger "pbr EGC-109: FOUND SRC: addr=$addr; src_addr=$src_addr; filtered_value_src_addr_tot=$filtered_value_src_addr_tot"
		if ! str_contains_word "$filtered_value_src_addr_tot" "$addr"; then
			processSrcFail="$processSrcFail $addr"
			#logger "pbr EGC-119: FOUND SRC:  ERROR in $processSrcFail"
		fi
	done

	if [ -n "$processPolicyError" ] || [ -n "$processSrcFail" ] || [ -n "$processDestFail" ]; then
		output_fail
		if [ -n "$processSrcFail" ]; then
			logger -p user.err -t "   $packageName" "ERROR: Policy $name failing with error in source $processSrcFail."
		fi
		if [ -n "$processDestFail" ]; then
			logger -p user.err -t "   $packageName" "ERROR: Policy $name failing with error in destination $processDestFail."
		fi
	else
		output_ok
	fi
