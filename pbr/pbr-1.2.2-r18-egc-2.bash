...
readonly PKG_VERSION='1.2.2-r18'
...
pbr_get_gateway4() {
	...
	if [ -z "$gw" ] || [ "$gw" = '0.0.0.0' ]; then
		gw="$(ip -4 route show dev "$dev" table all 2>/dev/null | awk '$1=="default" {for (i=1; i<NF; i++) if ($i=="via") {print $(i+1); exit}}')"    # use table all in case of netifd where default routes are not present in main table
		...
		{ [ -z "$gw" ] && ! ip address show dev "$dev" 2>/dev/null | grep -q "POINTOPOINT"; } && json add warning 'warningInterfaceRoutingUnknownGateway' 'nodevice'
	fi
	eval "$1"='$gw'
}
pbr_get_ipaddr4() {
...
pbr_get_gateway6() {
	...
	if [ -z "$gw" ] || [ "$gw" = '::/0' ] || [ "$gw" = '::0/0' ] || [ "$gw" = '::' ]; then
		gw="$(ip -6 route show dev "$dev" table all 2>/dev/null | awk '$1=="default" {for (i=1; i<NF; i++) if ($i=="via") {print $(i+1); exit}}')"    # use table all in case of netifd where default routes are not present in main table
		# Fall back in case interfaces do not have a default route
		[ -z "$gw" ] && gw="$(ip -6 route show dev "$dev" table all 2>/dev/null | awk '{for (i=1; i<=NF; i++) if ($i=="via") {print $(i+1); exit}}')"
		# Fall back to using ip -6 get route is not working as source routing is used which needs "from"
...
is_punycode() { echo "$1" | grep -qE '^([a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?$' && echo "$1" | grep -qE '(^|\.)xn--'; }
is_domain() { ! is_ipv4 "$1" && ! is_mac_address_bad_notation "$1" && { is_host "$1" || is_hostname "$1" || is_punycode "$1"; }; }
...
get_text() {
	...
	case "$r" in
		...
		warningInterfaceRoutingUnknownGateway) printf "Unknown Gateway for device '%s'" "$1";;
...
dns_policy_process() {
	...
	# Record first v4 and first v6 DNS server from the interface; the family
	# decision is made later by the filter_options loop, which knows the
	# per-group family by construction. Avoids passing a multi-value
	# src_addr to is_family_mismatch (which expected one address per call).
...
json_add_gateway() {
	...
	#[ $gw4 = "-" ] && gw4=0
	#[ $gw6 = "-" ] && gw6=0
	echo "egc-pbr action="$1" tid="$2" mark="$3" iface="$4" gw4="$gw4" dev4="$dev4" gw6="$gw6" dev6="$dev6" priority="$9" default="${10}""
	...
	case "$action" in
		...
		create)
			...
			if interface_routing 'create' "$_tid" "$_mark" "$iface" "$gw4" "$dev4" "$gw6" "$dev6" "$_priority"; then
				if [ -z "$dev4" ] && [ -z "$dev6" ]; then return 0; fi
				...
				echo "egc-pbr: gatewaySummary=$gatewaySummary"
...
start_service() {
	...
	# Build ifacesTriggers BEFORE is_wan_up â€” enumerate_interface only
	# consults UCI/static state and is safe with WAN down. Doing it here
	# means service_triggers() can register the full interface trigger
	# set even on a degraded start (pbrBootFlag=1), so interface events
	# during a WAN flap continue to fire pbr instead of having all
	# triggers replaced by a single boot-retry trigger.
	...
	echo "pbr-egc: reloadedIface=$reloadedIface"
	...
	# only on_start at error
	#if [ -n "$(ubus_get_status error)" ]; then
		serviceStartTrigger='on_start'
		unset reloadedIface
	...
	if [ "$serviceStartTrigger" = 'on_interface_reload' ] && is_uplink6 "$reloadedIface"; then
		echo "pbr-egc: reloadedIface=$reloadedIface; Starting 1"
		local prevGwIpv6
	...
	echo "pbr-egc: ADD JSON gatewaySummary=$gatewaySummary"
	if [ -n "$gatewaySummary" ]; then json_add_string 'gateways-egc' "$gatewaySummary"; else json_add_error 'errorNoGateways'; fi
	json_close_object
	json_add_array 'errors'
...
service_triggers() {
	local n
	# Always register the full validators + interface/config triggers.
	# procd's _procd_close_service unconditionally emits a `triggers`
	# field, and procd_update() replaces s->trigger with whatever we
	# emit â€” so emitting fewer triggers in any branch silently wipes
	# the rest. Registering the full set in every start_service path
	# preserves trigger registration across WAN-down transitions.
	...
	procd_open_trigger
		...
		# Safety net: when WAN is down at start time, also schedule a
		# raw boot-retry on iface.*.up. The interface triggers above
		# will normally drive recovery; the raw trigger only matters if
		# no interfaces were enumerable yet.
...
print_config_masked() {
	...
	| awk '
...

		{
			...
			# RFC1918/loopback addresses are not sensitive â€” skip masking them
