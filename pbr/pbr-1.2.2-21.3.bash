...
readonly PKG_VERSION='1.2.2-r21.3'
...
pbr_get_gateway4() {
	...
	if [ -z "$gw" ] || [ "$gw" = '0.0.0.0' ]; then
		...
		#{ [ -z "$gw" ] && ! is_p2p && ! is_xfrm; } && json add warning 'warningInterfaceRoutingUnknownGateway' "IPv4 interface:[$iface]; device:[$dev]"
	fi
	eval "$1"='$gw'
}
pbr_get_ipaddr4() {
...
pbr_get_gateway6() {
	...
	if [ -z "$gw" ] || [ "$gw" = '::/0' ] || [ "$gw" = '::0/0' ] || [ "$gw" = '::' ]; then
		...
		#{ [ -z "$gw" ] && ! is_p2p && ! is_xfrm; } && json add warning 'warningInterfaceRoutingUnknownGateway' "IPv6 interface:[$iface]; device:[$dev]"
	fi
	eval "$1"='$gw'
}
pbr_get_ipaddr6() {
