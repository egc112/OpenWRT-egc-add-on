...
version="1.05, 15-june-2024, by egc"
...
ip_leak_test(){
	...
	echo -e "\n  IPv4:$(echo "$ipv4" | grep '"ip":'); $(echo "$ipv4" | grep '"city":'); $(echo "$ipv4" | grep '"country":')"
	ipv6="$(uclient-fetch -6 --no-check-certificate -qO- http://v6.ipinfo.io/json)"
	if [[ -n "$ipv6" ]]; then
		echo -e "\n  IPv6:$(echo "$ipv6" | grep '"ip":'); $(echo "$ipv6" | grep '"city":'); $(echo "$ipv6" | grep '"country":')"
	else
