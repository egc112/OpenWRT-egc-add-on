...
readonly PKG_VERSION='1.2.1-r97'
...
mwan4_get_iface_list() {
	...
	# Use uci to get all interface sections where enabled=1
	for iface in $(uci -q show mwan4 | grep "=interface$" | cut -d. -f2 | cut -d= -f1); do
		if [ "$(uci -q get mwan4."$iface".enabled)" = "1" ]; then
...
mwan4_get_policy_list() {
	# Get list of mwan4 policies from UCI config
	# Output: space-separated list of policy names
	local policy_list=""
	mwan4_is_installed || return 1

	# Use uci to get all policy sections
	for policy in $(uci -q show mwan4 | grep "=policy$" | cut -d. -f2 | cut -d= -f1); do
...
mwan4_get_iface_nft_sets() {
	...
	family=$(uci -q get mwan4."$iface".family)
...
mwan4_get_policy_chain() {
	# Get the nftables policy chain name for a specific mwan4 policy
	# $1 = policy name
	# $2 = family (ipv4 or ipv6, defaults to ipv4)
	# Output: chain name (e.g., "mwan4_policy_balanced_ipv4")
	local policy="$1"
	local family="${2:-ipv4}"
	[ -z "$policy" ] && return 1
	mwan4_is_running || return 1
	...
	# Check if the chain exists in nftables
	if "$nft" list chain inet fw4 "mwan4_policy_${policy}_${family}" >/dev/null 2>&1; then
		echo "mwan4_policy_${policy}_${family}"
		return 0
	fi
	return 1
}
...
mwan4_get_mmx_mask() {
	...
	local mask
	mask=$(uci -q get mwan4.globals.mmx_mask)
	[ -z "$mask" ] && mask="0x3F00"  # Default mask
	echo "$mask"
}
...
stop_forward() {
	if [ -n "$stop_wan_leak" ] && [ "$(cat /proc/sys/net/ipv4/ip_forward)" != "0" ]; then
...
enable_forward() {
	if [ "$(cat /proc/sys/net/ipv4/ip_forward)" != "1" ]; then
		...
		# reloading of firewall is necessary to re-read sysctl
...
load_package_config() {
	...
	config_get      stop_wan_leak             'config' 'stop_wan_leak'            '0'
	...
	[ "$stop_wan_leak" = '1' ]        || unset stop_wan_leak
	...
	if [ "$iface" = 'all' ]; then
		case "$action" in
			...
			create_global_rules)
				...
				try ip -4 rule add lookup 'main' suppress_prefixlength "$prefixlength" || ipv4_error=1
				if [ -n "$ipv6_enabled" ]; then
					ip -6 rule del lookup 'main' suppress_prefixlength "$prefixlength" >/dev/null 2>&1
					try ip -6 rule add lookup 'main' suppress_prefixlength "$prefixlength" || ipv6_error=1
				fi
				return 0
			;;
		esac
...
restart() {
	...
	stop_forward
	stop
	# it apparantely takes time before routes are cleaned up, if started imediately a leak can occur
	[ -n "$stop_wan_leak" ] && sleep 2
	start
	enable_forward
}
