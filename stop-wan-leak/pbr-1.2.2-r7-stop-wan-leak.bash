...
#is_ovpn() { local d; uci_get_device d "$1" || uci_get_dev d "$1"; [ "${d:0:3}" = "tun" ] || [ "${d:0:3}" = "tap" ] || [ -f "/sys/devices/virtual/net/${d}/tun_flags" ]; }
is_ovpn() { local d; uci_get_device d "$1"; network_get_protocol p "$1"; [ "${d:0:3}" = "tun" ] || [ "${d:0:3}" = "tap" ] || [ "${p:0:7}" = "openvpn" ] || [ -f "/sys/devices/virtual/net/${d}/tun_flags" ]; }
...
service_triggers() {
	...
	if [ -n "$pbrBootFlag" ]; then
		...
		procd_open_trigger
			...
			procd_add_config_trigger "config.change" "network" "/etc/init.d/${packageName}" reload # needs increasing procd_reload_delay
